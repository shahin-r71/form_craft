  ...
  useEffect(() => {
    ...
    const fetchComments = async () => {
      try {
        ...
        const response = await fetch(`/api/templates/${id}/comments?page=${currentPage}&limit=5`);
        if (response.ok) {
          const data = await response.json();
  ...
  const handleComment = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setComment('');
        // Refresh comments
        const commentsResponse = await fetch(`/api/templates/${id}/comments?page=1&limit=5`);
        ...
        <div className="space-y-6">
          <div className="flex gap-4">
            ...
            <Button
              ...
              className={`transition-colors duration-200 ${hasLiked ? 'bg-red-50 text-red-600 hover:bg-red-100 border-red-200' : 'hover:bg-gray-100'}`}
            >
              <Heart
                className={`mr-2 h-4 w-4 transition-transform duration-200 ${hasLiked ? 'fill-current scale-110' : 'scale-100'}`}
              />
              <span className={`${hasLiked ? 'text-red-100' : 'text-gray-400'}`}>
                {template._count?.likes || 0} {template._count?.likes === 1 ? 'Like' : 'Likes'}
              </span>
            </Button>
          </div>
